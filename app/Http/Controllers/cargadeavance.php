...
class cargadeavance extends Controller
{
...
$idUser = $request->get('fk_users_id');
  ...
  $path = $request->file('file')->store('evidencia');
   ...
   $avance=avance::where("id",$id)->first();
   ...
   return redirect("cargadeavance?fk_users_id=$idUser")->with('actualizo', 'datos ');
  }else{
   $path =null;
   ...
   $avance->save();
  return redirect("cargadeavance?fk_users_id=$idUser")->with('actualizo');
...
public function create()
{
  ...
  $carga_avance = DB::TABLE('carga_avance')
...
public function deleteavance($id){
      ...
      $Avance = DB::TABLE('carga_avance')
->leftJoin ('users','carga_avance.fk_users_id','=','users.id')

      ->select (
         'carga_avance.fk_users_id',
         'carga_avance.id')
...
//->orWhere('carga_avance.meta', 'like',"%$nombre%")
