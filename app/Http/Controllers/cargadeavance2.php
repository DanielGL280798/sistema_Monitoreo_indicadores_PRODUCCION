...
class cargadeavance2 extends Controller
{
...
$idUser = $request->get('fk_users_id');
  ...
  $path = $request->file('file')->store('evidencia');
   ...
   $avance=avance2::where("id",$id)->first();
   ...
   return redirect("cargadeavance2?fk_users_id=$idUser")->with('actualizo', 'datos ');
  }else{
   $path =null;
   ...
   $avance->save();
  return redirect("cargadeavance2?fk_users_id=$idUser")->with('actualizo');
...
public function create()
{
  ...
  $carga_avance = DB::TABLE('carga_avance2')
...
public function deleteavance($id){
      ...
      $avance = avance2::find($id);
      $avance->delete();

      $Avance = DB::TABLE('carga_avance2')
->leftJoin ('users','carga_avance2.fk_users_id','=','users.id')

      ->select (
         'carga_avance2.fk_users_id',
         'carga_avance2.id')
...
public function edit(cargadeavance2 $cargadeavance2,Request $request)
{
...
//->orWhere('carga_avance2.meta', 'like',"%$nombre%")
